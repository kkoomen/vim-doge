...
export class PhpParserService extends BaseParserService implements CustomParserService {
      ...
      switch (node.type) {
        ...
        case NodeType.METHOD_DECLARATION: {
          this.result = {
            ...
            parameters: [],
            returnType: null,
            isNoConstructorMethod: node.children.filter((n: SyntaxNode) => n.type === 'name').shift()?.text !== '__construct',
          };
  ...
  private resolveFQN(type: string): string | null {
    let fqn = null;
    this.rootNode.children.filter((n: SyntaxNode) => n.type === 'namespace_use_declaration').forEach((n: SyntaxNode) => {
      n.children.filter((cn: SyntaxNode) => cn.type === 'namespace_use_clause').forEach((namespaceNode: SyntaxNode) => {
        const isAlias: boolean = namespaceNode.children.filter((cn: SyntaxNode) => cn.type === 'namespace_aliasing_clause').length > 0
        if (!isAlias) {
  ...
  private getClassPropertyTypeViaConstructor(node: SyntaxNode): string | undefined {
    const propertyName: string | undefined = node
      ?.children
    ...
    const constructorNode: SyntaxNode | undefined = node
      ?.parent
      ?.children
    ...
    constructorNode
      ...
      .shift()
      ?.children
      .filter((n: SyntaxNode) => n.type === 'expression_statement')
      .forEach((n: SyntaxNode) => {
        const expr: SyntaxNode = n.children.filter((cn: SyntaxNode) => cn.type === 'assignment_expression').shift() as SyntaxNode;
        const propName: string | undefined = expr.children.filter((cn: SyntaxNode) => cn.type === 'member_access_expression').shift()?.children.pop()?.text;
    ...
    const paramType: string | undefined = constructorNode
      ?.children
      ...
      .filter((n: SyntaxNode) => {
        const name: string | undefined = n.children.filter((cn: SyntaxNode) => cn.type === 'variable_name').shift()?.text;
        return name === paramName;
      })
      .shift()
      ?.children
      .filter((n: SyntaxNode) => ['optional_type', 'type_name', 'primitive_type'].includes(n.type))
      .shift()
      ?.text;
      ...
      switch (childNode.type) {
        ...
        case 'formal_parameters': {
          childNode.children
            .filter((n: SyntaxNode) => n.type === 'simple_parameter')
            .forEach((n: SyntaxNode) => {
              const param: Record<string, any> = { name: null, type: null, default: null, fqn: null };
